...
function configuredRoot(): string {
  return realpathSync(resolve(process.env.AMAZON_KINDLE_MCP_FILE_ROOT || process.cwd()));
}

export function resolveMcpFile(value: string): string {
  const candidate = realpathSync(resolve(value));
  if (process.env.AMAZON_KINDLE_MCP_ALLOW_ARBITRARY_FILES === "1") return candidate;
...
export function resolveOptionalMcpFile(value: string | undefined): string | undefined {
